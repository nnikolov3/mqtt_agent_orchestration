...
func NewRoleWorkerApp(workerID string, role types.WorkerRole, mqttHost string, mqttPort int, qdrantURL string) *RoleWorkerApp {
	...
	// Create RAG service
	ragService := rag.NewService("qdrant", qdrantURL)
	
	// Create role-based processor
	processor := worker.NewRoleBasedProcessor(role, ragService)
...
func main() {
	...
	app := NewRoleWorkerApp(*workerID, workerRole, *mqttHost, *mqttPort, *qdrantURL)
